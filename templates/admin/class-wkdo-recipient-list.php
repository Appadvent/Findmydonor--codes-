 ...
 * Donor list.
 */
class Wkdo_Recipient_List extends WP_List_Table {
	/**
	...
	private function wksa_table_data() {
		...
		if ( ! empty( $data ) ) {
			...
			foreach ( $data as $value ) {
				...
				if( !empty($all_data['wkdo_first_name']) ) {
					$name = $all_data['wkdo_first_name'] . ' '. $all_data['wkdo_last_name'];
				} else{
					$name = $user_data->display_name;
				}


				$staus = get_user_meta( $user_id, 'wkdo_becom_donor_status', true );
				if ( 'pending' === $staus ) {
					$user_status_html = "<a href='" . admin_url( 'admin.php?page=all-recipient&status=approved&did=' . $user_id ) . "'  class='button butn-green' /> Pending To Approved </a> ";
				} elseif ( 'approved' === $staus ) {
					$user_status_html = "<a href='" . admin_url( 'admin.php?page=all-recipient&status=pending&did=' . $user_id ) . "' class='button butn-orange'  /> Approved To Pending </a> ";
				} elseif ( 'rejected' === $staus ) {
					$user_status_html = "<a href='" . admin_url( 'admin.php?page=all-recipient&status=pending&did=' . $user_id ) . "' class='button butn-danger' />  Rejected To Pending </a> ";
				}

				$user_list[ $user_id ] = array(
					'ID'         => 'R100'.$value->ID,
					'status'     => $user_status_html,
					'user_login' => ! empty( $all_data['wkdo_first_name'] ) ? $all_data['wkdo_first_name'] : get_user_meta( $user_id, 'first_name', true ),
					'last'       => ! empty( $all_data['wkdo_last_name'] ) ? $all_data['wkdo_last_name'] : get_user_meta( $user_id, 'last_name', true ),
	...
	public function column_user_login( $item ) {
		$actions = array(
			'edit'   => sprintf( '<a href="admin.php?page=all-recipient&action=edit_recipient&pid=%s&section=edit_donor">' . __( 'Edit', 'wkmp_seller_auction' ) . '</a>', $item['ID'] ),
			'manage' => sprintf( '<a href="admin.php?page=all-recipient&action=view_recipient&pid=%s&section=view_recipient" >' . __( 'Manage', 'wkmp_seller_auction' ) . '</a>', $item['ID'] ),
		);
